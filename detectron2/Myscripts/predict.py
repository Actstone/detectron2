...
image_metadata = MetadataCatalog.get("my_dataset_train")
image_metadata.set(thing_classes=["铁壳打火机","黑钉打火机","刀具","电源和电池","剪刀"])
flag = 0
for i in glob.glob(os.path.join(cfg.OUTPUT_DIR,"test2017/*.jpg")):
    ...
    cv2.imwrite(os.path.join(cfg.OUTPUT_DIR,str(flag)+".jpg"),pred_image)
    flag += 1
    if flag == 3:
        break